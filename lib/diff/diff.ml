(** Generic differential testing: OCaml codec vs d3t-generated C code.
    ...
    {!D3t.to_c_stubs} and {!D3t.to_ml_stub}). All diff logic is generic over any
    record codec. *)
...
let schema ~name ~codec ~c_read ~c_write ~equal =
  let wire_size = D3t.Codec.wire_size codec in
  {
    name;
    c_read;
    c_write;
    equal;
    codec_decode = D3t.Codec.decode codec;
    codec_encode = D3t.Codec.encode codec;
    wire_size;
  }
...
let roundtrip_struct s buf =
  match D3t.read_struct s buf with
  | Error e -> Error e
  | Ok ps -> D3t.write_struct s ps
