...
let gen_ml_k_type ppf ~type_name kinds =
  Fmt.pf ppf "(";
  ...
  List.iter
    (fun (s : Wire.Everparse.Raw.struct_) ->
      ...
      if kinds <> [] then begin
        ...
        Fmt.pf ppf "external %s_parse_k : %a -> bytes -> %s option@\n" lower
          (fun ppf () -> gen_ml_k_type ppf ~type_name:lower kinds)
          () lower;
        ...
        Fmt.pf ppf "external %s_parse_k : 'a -> bytes -> 'a option@\n" lower;
        Fmt.pf ppf "  = \"caml_wire_%s_parse\"@\n@\n" lower;
...
let to_ml_stub (s : Wire.Everparse.Raw.struct_) =
  ...
  if kinds <> [] then begin
    ...
    Fmt.pf ppf "external parse_k : %a -> bytes -> t option@\n"
      (fun ppf () -> gen_ml_k_type ppf ~type_name:"t" kinds)
      ();
    Fmt.pf ppf "  = \"caml_wire_%s_parse\"@\n@\n" lower;
    ...
    Fmt.pf ppf "external parse_k : 'a -> bytes -> 'a option@\n";
    Fmt.pf ppf "  = \"caml_wire_%s_parse\"@\n@\n" lower;
...
(* Generate the _ExternalTypedefs.h for EverParse output schemas.
   WIRECTX holds a pointer to a GC-rooted local, not a bare value. *)
