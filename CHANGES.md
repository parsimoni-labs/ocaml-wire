...
- The C FFI stubs generated by `Wire_stubs` (`wire_ffi.c`) now include only the
  EverParse headers instead of folding each codec's generated `.c` into one
  translation unit with `#include`. Compile every codec's `<Name>.c` and
  `<Name>_Fields.c` as its own translation unit and link them (for example with
  a `foreign_archives` rule, as `bench/` and `fuzz/diff/` now do). In return,
  codecs that share a synthesised type (an `enum`, or a refined-byte or
  element-wrapper struct) can be linked into one binary; previously that failed
  to compile with duplicate-definition errors, so full protocol stacks built
  from per-codec parsers (Ethernet, IPv4, TCP, ...) now link cleanly
  (#135, @samoht)
